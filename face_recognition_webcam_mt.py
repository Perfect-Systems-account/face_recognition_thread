...
#             _ret.append(
#                 FaceRecognition.trim_bounds((face.left(), face.top(), face.right(), face.bottom()), image.shape))
#
#         return _ret
#
#     @staticmethod
...
#             _raw_face_locations = [dlib.rectangle(location[0], location[1], location[2], location[3]) for location in
#                                    locations]
#
...
#         face_recognition_model_location = resource_filename(__name__,
...
#     password = '@s322s322'
#     username = 'admin'
#     video_capture = webcam_video_stream(
#         src=f'rtsp://{username}:{password}@{ip}:{port}/Streaming/Channels/102/ch1-s1?tcp').start()  # (src=0).start()
#
...
#     video_process = face_recognition_process(capture=video_capture,
