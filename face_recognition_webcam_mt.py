...
# WORK BEFORE I CHANGE TO CLASSES
...
#             _frame = self.capture.read()
#             # ret, _frame = self.capture.read()
#             # if not ret or _frame is None:
#             #     print("Failed to grab a frame from the video stream.")
#             #     continue
#
#             if not (self.fx == 0.0 and self.fy == 0.0):
...
#                         _name = self.known_names[first_match_index]
#
#                         # i added for similary precents
#                         # face_distance = face_recognition.face_distance([self.known_encodings[first_match_index]],face_encoding)
#                         # facial_similarity_percentage = 100 - (face_distance[0] * 100)
#                         #
#                         #
#                         # facial_similarity_percentages.append(facial_similarity_percentage)
#
#                     _face_names.append(_name)
#
#             else:
...
def main():
    ...
    known_face_encodings,known_face_names,known_face_ids= sql_adapter.load_known_faces_from_database()
    ...
    print(f'The length of known_face_ids is {len(known_face_ids)}')
    ...
    password = '@s322s322'
    username = 'admin'
    src = f'rtsp://{username}:{password}@{ip}:{port}/Streaming/Channels/102/ch1-s1?tcp'
    video_capture = WebcamVideoStream(src=src).start()#(src=0).start()
